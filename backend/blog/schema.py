...
class Query(graphene.ObjectType):
    ...
    def resolve_all_posts(root, info: ResolveInfo) -> QuerySet:
        return (
    ...
    def resolve_author_by_username(root, info: ResolveInfo, username: str) -> QuerySet:
        return models.Profile.objects.select_related("user").get(
            user__username=username
        )

    def resolve_post_by_slug(root, info: ResolveInfo, slug: str) -> QuerySet:
        return (
            models.Post.objects.prefetch_related("tags")
            .select_related("author")
            .get(slug=slug)
